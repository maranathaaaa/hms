...
export function DashboardPage() {
	...
	if (roleId === ROLE_ID.DOCTOR) return <DoctorDashboard actorPrefix={actorPrefix} />;
	if (roleId === ROLE_ID.RECEPTIONIST || roleId === ROLE_ID.ACCOUNTANT) {
		return <ReceptionistDashboard actorPrefix={actorPrefix} />;
	}
	return <AdminDashboard actorPrefix={actorPrefix} />;
}

function AdminDashboard({ actorPrefix }: { actorPrefix: string }) {
		...
		<div className="space-y-6">
			...
			{stats ? (
				<>
					...
					<ScheduleAndBilling stats={stats} actorPrefix={actorPrefix} />
				</>
			) : null}
		</div>
	);
...
function DoctorDashboard({ actorPrefix }: { actorPrefix: string }) {
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
				<Card className="lg:col-span-2">
					<CardHeader
						...
						actions={
							<Link
								to={`${actorPrefix}/appointments` as never}
			...
			<Card>
				<CardHeader
					...
					actions={
						<Link
							to={`${actorPrefix}/medical-records` as never}
							className="text-sm font-medium text-primary-600 hover:text-primary-700"
						>
							View all
						</Link>
...
function ReceptionistDashboard({ actorPrefix }: { actorPrefix: string }) {
	const { data, isLoading, error } = useQuery({
		queryKey: ["dashboard"],
		queryFn: () => api.get<{ data: DashboardStats }>("/api/dashboard"),
	});
...
function ScheduleAndBilling({
	stats,
	actorPrefix,
}: { stats: DashboardStats; actorPrefix: string }) {
	return (
		<div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
			<Card className="lg:col-span-2">
				<CardHeader
